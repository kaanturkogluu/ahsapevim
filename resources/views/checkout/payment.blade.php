...
@section('title', 'Güvenli Ödeme - AhşapEvim')
            ...
            <div class="w-full lg:w-1/3">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="text-sm font-bold text-gray-850">{{ $order->name }}</div>
                        <div class="text-xs text-gray-600 mt-0.5">{{ $order->phone }} | {{ $order->email }}</div>
                    </div>
                    
                    <div class="mb-6">
                        <div class="text-xs text-gray-500 uppercase font-bold tracking-wider mb-1">Teslimat Adresi</div>
                        <div class="text-sm text-gray-700 leading-relaxed bg-gray-50 p-3 rounded border border-gray-150">{{ $order->address }}</div>
                    </div>
                    
                    <div class="border-t border-gray-100 pt-4">
                        <div class="flex justify-between items-center">
                            <span class="text-base font-bold text-gray-800">Ödenecek Tutar</span>
                            <span class="text-2xl font-extrabold text-[#C87A53]">{{ number_format($order->total_amount, 2, ',', '.') }} TL</span>
                        </div>
                    </div>
                </div>
            </div>
