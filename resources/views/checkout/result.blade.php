    ...
    <div class="container mx-auto px-4">
        <div class="max-w-3xl mx-auto bg-white rounded-3xl shadow-lg border border-amber-100 p-6 md:p-10 text-center">
            @if(session('status') === 'success')
                ...
                @if($isEft)
                    <div class="w-20 h-20 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center text-4xl mx-auto mb-5 border border-amber-200 shadow-sm">
                        <i class="fa-solid fa-clock"></i>
                    </div>
                    
                    <h1 class="text-3xl md:text-4xl font-black text-gray-800 mb-2 font-serif">Sipariş Talebiniz Alındı!</h1>
                    <p class="text-xs md:text-sm text-gray-600 mb-6 leading-relaxed max-w-lg mx-auto">
                        Havale / EFT ile sipariş talebiniz oluşturulmuştur. Masif ahşap el işçiliği ürünlerinizin hazırlanmaya başlanması için lütfen ödemenizi aşağıdaki banka hesabımıza iletiniz.
                    </p>

                    <!-- Order Status Notification Callout (EFT) -->
                    <div class="bg-amber-50/90 border border-amber-300 rounded-2xl p-4 mb-8 text-left flex items-center gap-3.5 text-xs text-stone-700 shadow-sm">
                        <div class="w-10 h-10 rounded-xl bg-amber-600 text-white flex items-center justify-center text-lg shrink-0 shadow-sm">
                            <i class="fa-solid fa-building-columns"></i>
                        </div>
                        <div>
                            <strong class="font-extrabold text-gray-900 text-sm block">Havale / EFT Ödemesi Bekleniyor</strong>
                            <span class="text-gray-600 font-medium">Banka transferiniz kontrol edilip admin tarafından onaylandığında siparişiniz işleme alınacak, SMS ve mail ile bilgilendirileceksiniz.</span>
                        </div>
                    </div>
                @else
                    ...
                    <p class="text-xs md:text-sm text-gray-600 mb-6 leading-relaxed max-w-lg mx-auto">
                        Ödemeniz kartınızdan güvenle onaylandı ve masif ahşap el işçiliği ürünleriniz hazırlık sırasına alındı.
                    </p>

                    <!-- Order Status Notification Callout (Card) -->
                ...
                @if($resultOrder)
                    <div class="bg-amber-50/50 rounded-2xl p-6 mb-8 border border-amber-200/80 text-left space-y-4">
                        ...
                        <div class="pt-3 border-t border-amber-200/60 flex justify-between items-center text-xs">
                            <span class="font-bold text-gray-600">{{ $isEft ? 'Ödenecek Havale / EFT Tutarı:' : 'Toplam Ödenen Tutar:' }}</span>
