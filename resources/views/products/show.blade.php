        ...
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <div class="w-full lg:w-[52%] flex flex-col md:flex-row gap-3 md:gap-4">
                ...
                <div class="flex-1 flex flex-col w-full">
                    <div id="mainImageContainer" class="-mx-4 w-[calc(100%+2rem)] sm:mx-0 sm:w-full bg-white sm:bg-gray-50/80 border-0 sm:border sm:border-gray-200/80 rounded-none sm:rounded-2xl relative overflow-hidden flex items-center justify-center h-[460px] xs:h-[500px] sm:h-[540px] md:h-[580px] lg:h-[620px]">
                        <img id="mainProductImage" src="{{ $product->image ?: '/cerceve.png' }}" alt="{{ $product->name }}" class="w-full h-full object-contain mix-blend-multiply p-0 sm:p-2 md:p-3 transition-all duration-300 z-10">
                    </div>
            ...
            <div class="w-full lg:w-[28%]">
                <form id="addToCartForm" action="{{ url('/sepet/ekle') }}" method="POST" enctype="multipart/form-data" class="border border-gray-200 rounded-2xl p-5 bg-white shadow-sm sticky top-24 space-y-4" onsubmit="return confirmAddToCart(event)">
                    ...
                    @endphp
                    <div class="mb-4 rounded-xl p-3.5 border transition-all shadow-sm {{ $isSameDayShipping ? 'bg-emerald-50 border-emerald-300 text-emerald-950' : 'bg-amber-50 border-amber-300 text-amber-950' }}">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl {{ $isSameDayShipping ? 'bg-emerald-600' : 'bg-amber-600' }} text-white flex items-center justify-center shrink-0 shadow-sm">
                                <i class="fa-solid {{ $isSameDayShipping ? 'fa-bolt text-lg' : 'fa-truck-fast text-base' }}"></i>
                            </div>
                            <div class="flex-1">
                                <div class="text-xs font-black tracking-wide uppercase {{ $isSameDayShipping ? 'text-emerald-700' : 'text-amber-700' }}">
                                    {{ $isSameDayShipping ? '⚡ AYNI GÜN KARGODA!' : '📦 YARIN KARGODA' }}
                                </div>
                                <p class="text-[12px] font-bold leading-tight mt-0.5 {{ $isSameDayShipping ? 'text-emerald-900' : 'text-amber-900' }}">
