...
@section('title', 'Gelir Tablosu & İstatistikler')
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
        ...
        <div>
            ...
            <div class="text-2xl font-black text-emerald-600">{{ number_format($totalRevenue, 2, ',', '.') }} ₺</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
        ...
        <div>
            ...
            <div class="text-2xl font-black text-blue-600">{{ number_format($thisMonthRevenue, 2, ',', '.') }} ₺</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
        ...
        <div>
            ...
            <div class="text-2xl font-black text-[#C87A53]">{{ number_format($totalOrders) }}</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
        ...
        <div>
            ...
            <div class="text-2xl font-black text-purple-600">{{ number_format($totalProductsSold) }}</div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
                @foreach(array_reverse(range(0, 29)) as $i)
                    @php
                        $idx = 29 - $i;
                        $label = $last30Days[$i];
                    ...
                    <tr class="bg-white border-b border-gray-50 hover:bg-orange-50/20 transition">
                        <td class="px-6 py-3 text-gray-600 text-xs font-medium">{{ $label }}</td>
                        <td class="px-6 py-3 text-right font-black {{ $amount > 0 ? 'text-emerald-600' : 'text-gray-300' }}">
