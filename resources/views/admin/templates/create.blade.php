...
<div class="bg-gray-50 min-h-[calc(100vh-140px)] flex flex-col">
    <div class="container mx-auto px-4 py-6 flex-grow flex flex-col lg:flex-row gap-6">
        ...
        <div class="w-full lg:w-96 flex-shrink-0 flex flex-col gap-4">
            ...
            <form action="{{ route('admin.templates.store') }}" method="POST" id="templateForm" class="space-y-4">
                @csrf
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                <input type="range" name="bump_scale" id="bumpScale" min="0" max="0.5" step="0.01" value="0.08" class="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer" oninput="document.getElementById('bumpVal').innerText = this.value">
                                <span id="bumpVal" class="text-xs font-bold text-gray-600 w-8">0.08</span>
                            </div>
                        </div>
                    </div>
                </div>
...
function generateWoodTexture(woodType, rendererInstance) {
    ...
    // 3. Soft grain variation rings (horizontal waves)
    ...
    // 4. Sharp horizontal grain lines
