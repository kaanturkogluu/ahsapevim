...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center">
        ...
        <div>
            ...
            <div class="text-2xl font-bold text-green-600">{{ number_format(abs($totalCredit), 2, ',', '.') }} ₺</div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
        <form action="{{ route('admin.cari.index') }}" method="GET" class="flex gap-2">
            <input type="text" name="search" value="{{ $search }}" placeholder="Müşteri Adı, E-posta veya Tel" class="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#C87A53] focus:ring-1 focus:ring-[#C87A53] w-64">
            ...
            @if($search)
                <a href="{{ route('admin.cari.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm transition">Temizle</a>
            @endif
        </form>
    </div>
                ...
                @forelse($users as $user)
                    <tr class="bg-white border-b border-gray-50 hover:bg-orange-50/30 transition">
                        ...
                        <td class="px-6 py-4 text-right font-bold text-base">
                            @if($user->balance > 0)
                                <span class="text-red-600" title="Müşteri firmamıza borçlu">{{ number_format($user->balance, 2, ',', '.') }} ₺</span>
                                <div class="text-[10px] text-red-500 font-normal">Borçlu</div>
                            @elseif($user->balance < 0)
                                <span class="text-green-600" title="Müşteri alacaklı">{{ number_format(abs($user->balance), 2, ',', '.') }} ₺</span>
                        ...
                        <td class="px-6 py-4 text-center">
                            <a href="{{ route('admin.cari.show', $user->id) }}" class="inline-flex items-center gap-1 px-3 py-1.5 bg-[#C87A53] hover:bg-[#A65F38] text-white rounded-lg text-xs font-semibold transition shadow-sm">
