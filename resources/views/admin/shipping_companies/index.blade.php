...
@section('header', 'Kargo Şirketleri Yönetimi')
...
<div class="space-y-6 max-w-5xl">
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <form action="{{ route('admin.shipping_companies.store') }}" method="POST" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            @csrf
            <div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($companies as $company)
                        <tr class="hover:bg-gray-50/80 transition">
                            <td class="p-3 font-mono font-bold">#{{ $company->id }}</td>
                            <td class="p-3 font-bold text-gray-900">{{ $company->name }}</td>
                            <td class="p-3">
                                @if($company->website_url)
                                    <a href="{{ $company->website_url }}" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1">
                                        <i class="fa-solid fa-arrow-up-right-from-square text-[10px]"></i> {{ $company->website_url }}
                                    </a>
                                @else
                            ...
                            <td class="p-3">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $company->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $company->is_active ? 'Aktif' : 'Pasif' }}
                                </span>
                            </td>
                            <td class="p-3 text-right">
                                <form action="{{ route('admin.shipping_companies.destroy', $company->id) }}" method="POST" onsubmit="return confirm('Bu kargo şirketini silmek istediğinize emin misiniz?')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
