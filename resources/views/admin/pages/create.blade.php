...
<div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm max-w-4xl">
    <div class="mb-6 pb-4 border-b border-gray-100 flex justify-between items-center">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Eklenecek bilgilendirme sayfasının başlığını ve metin/HTML içeriğini tanımlayın.</p>
        </div>
    ...
    <form action="{{ route('admin.pages.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <div class="flex items-center">
                    ...
                    <input type="text" name="slug" value="{{ old('slug') }}" class="w-full text-sm border-gray-300 rounded-r-lg p-2.5 border focus:border-brand focus:ring-0 outline-none font-mono" placeholder="örn: kargo-koşullari">
                </div>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="content" rows="16" required class="w-full text-sm border-gray-300 rounded-lg p-3 border focus:border-brand focus:ring-0 outline-none font-mono leading-relaxed bg-gray-50/50" placeholder="Sayfa içeriğinizi buraya yazın veya HTML formatında yapıştırın.">{{ old('content') }}</textarea>
        </div>
