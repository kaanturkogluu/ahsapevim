...
@section('header', 'E-Posta Şablonu Düzenle - ' . $template->name)
...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.email_templates.index') }}" class="text-xs font-bold text-gray-500 hover:text-gray-700 transition flex items-center gap-1">
            <i class="fa-solid fa-arrow-left"></i> Şablon Listesine Dön
        </a>
        <a href="{{ route('admin.email_templates.preview', $template->id) }}" target="_blank" class="py-1.5 px-3 bg-stone-100 text-stone-700 hover:bg-stone-200 font-bold text-xs rounded-lg transition inline-flex items-center gap-1 border border-stone-300">
            <i class="fa-solid fa-eye"></i> Canlı Ön İzle
        </a>
    </div>

    @if(session('success'))
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg text-xs font-bold">
            <i class="fa-solid fa-check-circle mr-1"></i> {{ session('success') }}
        </div>
    @endif

    @if(session('error'))
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg text-xs font-bold">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p class="text-xs text-gray-500 mb-6 font-mono">Olay Kodu: {{ $template->slug }}</p>

            <form id="emailTemplateForm" action="{{ route('admin.email_templates.update', $template->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="subject" name="subject" value="{{ old('subject', $template->subject) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm font-semibold focus:border-[#C87A53] focus:ring-0 outline-none">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 bg-amber-50/60 p-3 rounded-xl border border-amber-200/80 mb-3">
                        @if(!empty($template->shortcodes))
                            @foreach($template->shortcodes as $code => $desc)
                                <button type="button" onclick="insertShortcode('{{ $code }}')" class="px-3 py-1.5 bg-white hover:bg-amber-100 text-[#C87A53] border border-amber-300 rounded-lg text-xs font-bold transition flex items-center gap-1 shadow-sm">
                                    <i class="fa-solid fa-plus text-[10px]"></i> {{ $desc }} <span class="font-mono text-[10px] opacity-75">({{ $code }})</span>
                                </button>
                            @endforeach
                        @endif
                    </div>
                ...
                <div>
                    ...
                    <!-- Hidden Real Input -->
                    <input type="hidden" id="content" name="content" value="{{ old('content', $template->content) }}">
                ...
                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ $template->is_active ? 'checked' : '' }} class="w-4 h-4 text-[#C87A53] rounded border-gray-300 focus:ring-0">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-amber-50/60 p-5 rounded-xl border border-amber-200/80">
                ...
                <div class="space-y-2">
                    @if(!empty($template->shortcodes))
                        @foreach($template->shortcodes as $code => $desc)
                            <div class="bg-white p-2.5 rounded-lg border border-amber-200/80 flex items-center justify-between text-xs cursor-pointer hover:bg-amber-100 transition" onclick="insertShortcode('{{ $code }}')">
                                <code class="font-bold text-[#C87A53] font-mono">{{ $code }}</code>
                                <span class="text-[11px] text-gray-600 font-semibold">{{ $desc }}</span>
                            </div>
                        @endforeach
                    @endif
                </div>
            ...
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                ...
                <form action="{{ route('admin.email_templates.test', $template->id) }}" method="POST" class="space-y-3">
                    @csrf
                    <div>
                        <label for="test_email" class="block text-[11px] font-bold text-gray-700 mb-1">E-Posta Adresiniz</label>
                        <input type="email" id="test_email" name="email" value="{{ auth()->user()->email }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs outline-none focus:border-[#C87A53]">
                    </div>
