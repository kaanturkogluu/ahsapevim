...
@section('header', 'Netgsm SMS Gönderim Logları & Manuel SMS Gönderim')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-200 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-4">
        <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
            <a href="{{ route('admin.sms_logs.index') }}" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition {{ !request('status') ? 'bg-[#29221C] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                Tüm SMS'ler
            </a>
            <a href="{{ route('admin.sms_logs.index', ['status' => 'success']) }}" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition {{ request('status') === 'success' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' }}">
                <i class="fa-solid fa-check text-[10px] mr-1"></i> Başarılılar
            </a>
            <a href="{{ route('admin.sms_logs.index', ['status' => 'failed']) }}" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition {{ request('status') === 'failed' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-700 hover:bg-rose-100' }}">
                <i class="fa-solid fa-xmark text-[10px] mr-1"></i> Başarısızlar
            </a>
        </div>

        <form action="{{ route('admin.sms_logs.index') }}" method="GET" class="flex items-center gap-2 w-full md:w-auto">
            @if(request('status'))
                <input type="hidden" name="status" value="{{ request('status') }}">
            @endif
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Telefon no, mesaj, kod..." class="w-full md:w-64 text-xs border border-gray-300 rounded-xl px-3 py-2 outline-none focus:border-[#C87A53]">
                ...
                <tbody class="divide-y divide-gray-100 font-medium text-gray-700">
                    @forelse($logs as $log)
                        <tr class="hover:bg-gray-50/70 transition">
                            ...
                            <td class="p-4 max-w-xs truncate" title="{{ $log->message }}">
                                {{ $log->message }}
                            </td>
                            ...
                            <td class="p-4 text-center font-mono text-xs">
                                <span class="px-2 py-0.5 bg-gray-100 rounded font-bold">{{ $log->response_code ?: 'N/A' }}</span>
                            </td>
                            <td class="p-4 text-center">
                            ...
                            <td class="p-4 text-right space-x-1 whitespace-nowrap">
                                <button type="button" onclick="viewSmsDetail({{ json_encode($log) }})" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-bold transition">
    ...
    <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-gray-200 relative">
        ...
        <form action="{{ route('admin.sms.send_manual') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="message" id="modalSmsBody" rows="4" maxlength="1000" required placeholder="Müşterinize gitmesini istediğiniz SMS metni..." class="w-full text-xs border border-gray-300 rounded-xl p-2.5 outline-none focus:border-[#C87A53]" oninput="updateSmsCharCount(this)"></textarea>
                <div class="text-[10px] text-gray-400 text-right mt-1 font-mono">
                    <span id="smsCharCount">0</span> Karakter
                </div>
            </div>
...
function updateSmsCharCount(el) {
    document.getElementById('smsCharCount').textContent = el.value.length;
}
