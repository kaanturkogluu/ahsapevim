...
<div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm max-w-5xl">
    ...
    <div class="mb-6 pb-4 border-b border-gray-100 flex flex-wrap justify-between items-center gap-4">
        ...
        <div class="flex items-center gap-3">
            <button type="button" onclick="openDeleteOrderModal(<?php echo e($order->id); ?>, 'Sipariş #<?php echo e($order->id); ?> (<?php echo e($order->name); ?>)')" class="py-1.5 px-3 bg-rose-50 text-rose-600 hover:bg-rose-100 font-extrabold text-xs rounded-lg transition inline-flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
            ...
            <div class="mt-4 pt-3 border-t border-gray-200/80 flex flex-wrap gap-2">
                <button type="button" onclick="openSendMailModal('<?php echo e($order->email); ?>', '<?php echo e($order->id); ?>')" class="py-1.5 px-2.5 bg-amber-100 text-amber-900 hover:bg-amber-200 font-extrabold text-[11px] rounded-lg transition inline-flex items-center gap-1 border border-amber-300">
                    <i class="fa-solid fa-paper-plane text-xs"></i> E-Posta Gönder
                </button>
                <button type="button" onclick="openSendSmsModal('<?php echo e($order->phone); ?>', '<?php echo e($order->id); ?>')" class="py-1.5 px-2.5 bg-emerald-100 text-emerald-900 hover:bg-emerald-200 font-extrabold text-[11px] rounded-lg transition inline-flex items-center gap-1 border border-emerald-300">
        ...
        <div class="bg-orange-50/50 p-4 rounded-xl border border-orange-100">
            ...
            <form action="<?php echo e(route('admin.orders.update', $order->id)); ?>" method="POST" class="space-y-3" onsubmit="preventSpamSubmit(this)">
                <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <select name="shipping_company_id" class="w-full text-xs border-gray-300 rounded-lg p-2 border focus:border-[#C87A53] outline-none">
                        ...
                        <?php $__currentLoopData = $shippingCompanies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $company): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($company->id); ?>" <?php echo e($order->shipping_company_id == $company->id ? 'selected' : ''); ?>><?php echo e($company->name); ?></option>
                ...
                <div>
                    ...
                    <input type="text" name="cargo_tracking_code" placeholder="Örn: 123456789" value="<?php echo e(old('cargo_tracking_code', $order->cargo_tracking_code)); ?>" class="w-full text-xs border-gray-300 rounded-lg p-2 border focus:border-[#C87A53] outline-none font-mono">
                </div>

                <?php if(!empty($order->cargo_tracking_code) || $order->status === 'completed'): ?>
    ...
    <?php if($order->status === 'failed' || $order->status === 'cancelled' || !empty($order->payment_error_reason)): ?>
        <div class="bg-rose-50 border border-rose-200 p-4 rounded-xl mb-6 shadow-2xs">
            ...
            <div class="text-xs text-rose-800 space-y-1">
                <p><strong>İptal / Başarısız Sebebi:</strong> <span class="font-extrabold text-rose-950"><?php echo e($order->payment_error_reason ?: 'Yetersiz Bakiye / Kart Onayı Alınamadı'); ?></span></p>
                <?php if($order->payment_id): ?>
                    <p class="font-mono text-[11px] text-rose-700"><strong>İşlem Referans No:</strong> <?php echo e($order->payment_id); ?></p>
                <?php endif; ?>
            </div>
        </div>
    <?php else: ?>
        <div class="bg-gradient-to-br from-emerald-50/70 via-white to-gray-50 p-4 rounded-xl border border-emerald-200/80 mb-6 shadow-2xs">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
                <div class="bg-white p-3 rounded-lg border border-gray-200/70 shadow-2xs">
                    ...
                    <span class="text-sm font-black text-gray-800 mt-0.5 block">₺<?php echo e(number_format($order->paid_price ?? $order->total_amount, 2, ',', '.')); ?></span>
                </div>
                ...
                <div class="bg-white p-3 rounded-lg border border-gray-200/70 shadow-2xs">
                    ...
                    <span class="text-sm font-black text-blue-700 mt-0.5 block">
                        <?php if(($order->installment ?? 1) > 1): ?>
                            <?php echo e($order->installment); ?> Taksit
                        <?php else: ?>
                            Tek Çekim (1 Taksit)
                        <?php endif; ?>
                    </span>
                ...
                <div class="bg-white p-3 rounded-lg border border-emerald-300 shadow-2xs bg-emerald-50/40">
                    ...
                    <span class="text-sm font-black text-emerald-700 mt-0.5 block">₺<?php echo e(number_format($order->merchant_payout_amount ?? $order->total_amount, 2, ',', '.')); ?></span>
                </div>
                ...
                <div class="bg-white p-3 rounded-lg border border-gray-200/70 shadow-2xs">
                    ...
                    <span class="text-xs font-bold text-gray-700 mt-0.5 block">
                        <?php echo e($order->card_family ?: 'Kredi Kartı'); ?> <?php echo e($order->card_last_four ? '**** ' . $order->card_last_four : ''); ?>

                    </span>
                    <span class="text-[10px] font-mono text-gray-400 block truncate" title="<?php echo e($order->payment_id); ?>"><?php echo e($order->payment_id ?: 'N/A'); ?></span>
                </div>
            </div>
        </div>
    <?php endif; ?>
                            ...
                            <?php if(!empty($item->features['is_gift']) || !empty($item->features['gift_note'])): ?>
                                <div class="mt-2 bg-amber-50/80 p-2.5 rounded-xl border border-amber-200 text-xs text-amber-950">
                                    ...
                                    <p class="mt-1 text-gray-800 italic bg-white p-2 rounded-lg border border-amber-200/60 font-serif text-xs">
                                        "<?php echo e($item->features['gift_note'] ?: 'Özel not belirtilmedi (Hediye Paketi Yapılacak)'); ?>"
                                    </p>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if($fImg || $bImg): ?>
                                <div class="mt-3 bg-white p-3 rounded-xl border border-orange-200/80 space-y-2">
                                    <span class="block text-[11px] font-extrabold text-[#C87A53] uppercase tracking-wider">
                                        ...
                                        <span class="px-2 py-0.5 rounded text-[10px] ml-1 <?php echo e($isDoubleFace ? 'bg-emerald-100 text-emerald-800' : 'bg-orange-100 text-orange-800'); ?>">
                                            <?php echo e($isDoubleFace ? 'Çift Yüzlü (2 Fotoğraf)' : 'Tek Yüzlü (1 Fotoğraf)'); ?>
...
<div id="sendMailModal" class="fixed inset-0 z-[99999] bg-black/70 backdrop-blur-xs hidden flex-col items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-gray-200 relative">
        ...
        <form action="<?php echo e(route('admin.mail.send_manual')); ?>" method="POST" class="space-y-4" onsubmit="preventSpamSubmit(this)">
            <?php echo csrf_field(); ?>
            <div>
...
<div id="sendSmsModal" class="fixed inset-0 z-[99999] bg-black/70 backdrop-blur-xs hidden flex-col items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-gray-200 relative">
        ...
        <form action="<?php echo e(route('admin.sms.send_manual')); ?>" method="POST" class="space-y-4" id="orderSmsForm">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/u111121823/domains/ahsapevimmanisa.com/public_html/resources/views/admin/orders/show.blade.php ENDPATH**/ ?>