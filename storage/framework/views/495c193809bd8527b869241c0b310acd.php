...
<div>
    ...
    <?php if(!empty($faqItems) && is_array($faqItems) && count($faqItems) > 0): ?>
        <div class="space-y-3" id="faqAccordion">
            <?php $__currentLoopData = $faqItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="faq-item border border-gray-200 rounded-xl overflow-hidden transition-all duration-200 bg-white hover:border-amber-300">
                    <button type="button" 
                            onclick="toggleFaqAccordion(<?php echo e($index); ?>)" 
                        ...
                        <span id="faqChevron-<?php echo e($index); ?>" class="w-8 h-8 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center shrink-0 transition-transform duration-300">
                            <i class="fa-solid fa-chevron-down text-xs"></i>
                        </span>
                    </button>
                    <div id="faqAnswer-<?php echo e($index); ?>" class="hidden px-5 pb-5 pt-1 text-gray-600 text-sm leading-relaxed border-t border-gray-100 bg-gray-50/50">
    ...
    <?php elseif(!empty($rawContent) && is_string($rawContent)): ?>
    ...
    <div class="mt-10 p-6 rounded-2xl bg-gray-900 text-white flex flex-col md:flex-row items-center justify-between gap-4 shadow-md">
        ...
        <a href="<?php echo e(url('/iletisim')); ?>" class="py-2.5 px-5 bg-[#C87A53] hover:bg-[#A65F38] text-white font-bold rounded-xl text-xs transition shrink-0 flex items-center gap-2">
...
<?php echo $__env->make('pages.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ahsapevim\resources\views/pages/faq.blade.php ENDPATH**/ ?>