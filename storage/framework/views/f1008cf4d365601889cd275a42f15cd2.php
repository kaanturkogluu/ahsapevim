<?php $__env->startSection('title', 'Favorilerim — AhşapEvim'); ?>
...
<div class="bg-gray-50 pb-16 min-h-screen pt-8">
    <div class="container mx-auto px-4">
        ...
        <div class="flex items-center justify-between mb-8 pb-4 border-b border-amber-100">
            ...
            <a href="<?php echo e(url('/urunler')); ?>" class="text-xs font-bold text-[#C87A53] hover:underline flex items-center gap-1">
        ...
        <?php if($favorites->isEmpty()): ?>
            <div class="bg-white rounded-2xl shadow-sm p-12 text-center border border-amber-100 max-w-lg mx-auto">
                ...
                <a href="<?php echo e(url('/urunler')); ?>" class="inline-block bg-[#C87A53] hover:bg-[#A65F38] text-white font-bold py-3 px-8 rounded-xl transition shadow-md text-sm">
                    Ürünleri Keşfet
                </a>
            </div>
        <?php else: ?>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6" id="favoritesGrid">
                <?php $__currentLoopData = $favorites; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="product-card group relative flex flex-col justify-between" id="favCard-<?php echo e($product->id); ?>">
                        
                        
                        <button type="button" onclick="toggleFavorite(<?php echo e($product->id); ?>, this)" class="absolute top-3 right-3 z-20 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-red-500 shadow-md hover:scale-110 transition" title="Favorilerden Çıkar">
                        ...
                        <?php if($product->discount_percent > 0): ?>
                            <div class="absolute top-3 left-3 z-20 bg-red-600 text-white font-extrabold text-[10px] px-2 py-0.5 rounded-full shadow">
                                %<?php echo e($product->discount_percent); ?> İNDİRİM
                            </div>
                        <?php endif; ?>

                        <a href="<?php echo e(url('/urun/' . $product->id)); ?>" class="block p-4">
                            <div class="aspect-square w-full rounded-xl bg-stone-100 overflow-hidden mb-3 relative flex items-center justify-center p-2">
                                <img src="<?php echo e($product->image ?: '/cerceve.png'); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-contain mix-blend-multiply group-hover:scale-105 transition duration-300">
                            </div>

                            <span class="text-[10px] font-bold text-amber-700 uppercase tracking-wider block mb-1">
                                <?php echo e($product->category->name ?? 'Ahşap'); ?>
                            ...
                            <div class="mt-3 flex items-baseline gap-2">
                                ...
                                <?php if($product->original_price && $product->original_price > $product->price): ?>
                                    <span class="text-[11px] text-gray-400 line-through">
                                        <?php echo e(number_format($product->original_price, 2, ',', '.')); ?> TL
                                    </span>
                                <?php endif; ?>
                            </div>
                        </a>
                        ...
                        <div class="p-4 pt-0">
                            <a href="<?php echo e(url('/urun/' . $product->id)); ?>" class="w-full py-2.5 bg-amber-50 hover:bg-[#C87A53] text-[#C87A53] hover:text-white font-bold text-xs rounded-xl transition flex items-center justify-center gap-1.5 border border-amber-200 hover:border-[#C87A53]">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\cerceve\resources\views/favorites/index.blade.php ENDPATH**/ ?>