<?php $__env->startSection('title', 'Sipariş Takip - AhşapEvim'); ?>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <?php if(session('error')): ?>
            <div class="max-w-xl mx-auto bg-red-100 border border-red-400 text-red-700 px-5 py-4 rounded-xl text-xs font-bold mb-8 text-center shadow-sm">
                <i class="fa-solid fa-circle-exclamation text-base mr-1"></i> <?php echo e(session('error')); ?>
        ...
        <?php if(!isset($order)): ?>
            ...
            <div class="max-w-xl mx-auto bg-white rounded-2xl shadow-md border border-amber-150 p-6 md:p-8 mb-8">
                <form action="<?php echo e(route('order.tracking.search')); ?>" method="POST" class="space-y-5">
                    <?php echo csrf_field(); ?>
                    ...
                    <div>
                        ...
                        <input type="text" id="tracking_code" name="tracking_code" value="<?php echo e(old('tracking_code')); ?>" placeholder="Örn: AHS-849201 veya Sipariş No (#12)" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-[#C87A53] focus:ring-0 outline-none text-sm font-mono uppercase transition">
                    ...
                    <div>
                        ...
                        <input type="text" id="phone_or_email" name="phone_or_email" value="<?php echo e(old('phone_or_email')); ?>" placeholder="Siparişte girdiğiniz e-posta veya telefon..." required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-[#C87A53] focus:ring-0 outline-none text-sm transition">
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-100 text-center">
                    ...
                    <a href="<?php echo e(route('login')); ?>" class="inline-flex items-center gap-2 bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold px-5 py-2.5 rounded-xl text-xs transition border border-stone-200">
        ...
        <?php else: ?>
            ...
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <div class="flex justify-between items-center">
                    <a href="<?php echo e(route('order.tracking')); ?>" class="text-xs font-bold text-gray-600 hover:text-brand transition flex items-center gap-1">
                    ...
                    <span class="text-xs font-bold bg-amber-100 text-amber-900 px-3 py-1 rounded-full">
                        Takip Kodu: <strong class="font-mono uppercase"><?php echo e($order->tracking_code ?: 'AHS-'.$order->id); ?></strong>
                    </span>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-amber-100">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-gray-100">
                        <div>
                            ...
                            <h2 class="text-xl font-black text-gray-800">#<?php echo e($order->id); ?> - <?php echo e($order->tracking_code ?: 'AHS-'.$order->id); ?></h2>
                            <span class="text-xs text-gray-500 mt-1 block">Tarih: <?php echo e($order->created_at?->format('d.m.Y H:i') ?? '-'); ?></span>
                        </div>
                    ...
                    <div class="pt-6 grid grid-cols-3 gap-2 text-center relative">
                        ...
                        <div class="flex flex-col items-center">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm mb-2 shadow-sm <?php echo e(in_array($order->status, ['paid', 'preparing', 'shipped', 'completed', 'delivered', 'teslim_edildi', 'teslimedildi']) ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-500'); ?>">
                        ...
                        <!-- Step 2 -->
                        <div class="flex flex-col items-center">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm mb-2 shadow-sm <?php echo e(in_array($order->status, ['shipped', 'completed', 'delivered', 'teslim_edildi', 'teslimedildi']) ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-500'); ?>">
                        ...
                        <!-- Step 3 -->
                        <div class="flex flex-col items-center">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm mb-2 shadow-sm <?php echo e(in_array($order->status, ['completed', 'delivered', 'teslim_edildi', 'teslimedildi']) ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-500'); ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-amber-100 md:col-span-1 space-y-3 text-xs text-gray-700">
                        ...
                        <p><strong>Alıcı:</strong> <?php echo e($order->name); ?></p>
                        <p><strong>Telefon:</strong> <?php echo e($order->phone); ?></p>
                        <p class="whitespace-pre-line bg-gray-50 p-2.5 rounded-lg border border-gray-150"><?php echo e($order->address); ?></p>
                        <p><strong>Şehir:</strong> <?php echo e($order->city ?: 'Manisa'); ?> / <?php echo e($order->district ?: 'Merkez'); ?></p>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-amber-100 md:col-span-2">
                        <h3 class="text-sm font-extrabold text-gray-800 pb-2 border-b border-gray-100 mb-4 flex items-center justify-between">
                            ...
                            <span class="text-xs font-bold text-[#C87A53]">Toplam: ₺<?php echo e(number_format($order->total_amount, 2, ',', '.')); ?></span>
                        </h3>
                                ...
                                <div class="py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-3">
                                        <?php if($item->product && $item->product->image): ?>
                                            <img src="<?php echo e(url($item->product->image)); ?>" class="w-12 h-12 object-cover rounded-lg border border-gray-200">
                                        <?php else: ?>
                                        ...
                                        <div>
                                            <h4 class="text-xs font-bold text-gray-800"><?php echo e($item->product ? $item->product->name : 'Ahşap Çerçeve'); ?></h4>
                                            <div class="text-[11px] text-gray-500 mt-0.5">
                                                Adet: <strong><?php echo e($item->quantity); ?></strong> × ₺<?php echo e(number_format($item->price, 2, ',', '.')); ?>
                                            ...
                                            <?php if($fImg || $bImg): ?>
                                                <div class="mt-1 flex items-center gap-2">
                                                    <?php if($fImg): ?>
                                                        <a href="<?php echo e(str_starts_with($fImg, 'http') ? $fImg : url($fImg)); ?>" download target="_blank" class="inline-flex items-center gap-1 bg-amber-50 text-amber-900 border border-amber-200 text-[10px] font-bold px-2 py-0.5 rounded hover:bg-amber-100 transition">
                                                            <i class="fa-solid fa-image text-amber-600"></i> 1. Ön Yüz Fotoğrafı
                                                        </a>
                                                    <?php endif; ?>
                                                    <?php if($bImg): ?>
                                                        <a href="<?php echo e(str_starts_with($bImg, 'http') ? $bImg : url($bImg)); ?>" download target="_blank" class="inline-flex items-center gap-1 bg-emerald-50 text-emerald-900 border border-emerald-200 text-[10px] font-bold px-2 py-0.5 rounded hover:bg-emerald-100 transition">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ahsapevim\resources\views\pages\order_tracking.blade.php ENDPATH**/ ?>