    ...
    <div class="container mx-auto px-4 py-4">
        ...
        <div class="mt-16">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-3 hover:shadow-lg transition-all product-card flex flex-col h-full group relative">
                    <a href="<?php echo e(url('/urun/' . $simProduct->id)); ?>" class="block mb-3 relative overflow-hidden rounded-lg bg-gray-50 pt-[100%]">
                    ...
                    <div class="flex-grow flex flex-col">
                        ...
                        <h3 class="text-sm font-medium text-gray-800 mb-2 line-clamp-2 leading-tight group-hover:text-brand transition"><a href="<?php echo e(url('/urun/' . $simProduct->id)); ?>"><?php echo e($simProduct->name); ?></a></h3>
        ...
        <div class="mt-16">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-3 hover:shadow-lg transition-all product-card flex flex-col h-full group relative">
                    <a href="<?php echo e(url('/urun/' . $recentProduct->id)); ?>" class="block mb-3 relative overflow-hidden rounded-lg bg-gray-50 pt-[100%]">
                    ...
                    <div class="flex-grow flex flex-col">
                        <h3 class="text-sm font-medium text-gray-800 mb-2 line-clamp-2 leading-tight group-hover:text-brand transition"><a href="<?php echo e(url('/urun/' . $recentProduct->id)); ?>"><?php echo e($recentProduct->name); ?></a></h3>
                        <div class="mt-auto pt-2 flex items-center justify-between">
...
<div id="customizationModal" class="fixed inset-0 z-[99999] bg-black/90 hidden flex-col items-center justify-center p-4 md:p-8 backdrop-blur-sm">
    <div class="bg-white w-full max-w-4xl rounded-2xl overflow-hidden flex flex-col h-[90vh] shadow-2xl relative">
        
        <!-- Header -->
        <div class="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50">
            <div>
                <h3 class="text-xl font-bold text-gray-800" id="modalTitle">3D Fotoğraf Yükle ve Ön İzle</h3>
                <p class="text-sm text-gray-500" id="modalSubtitle">Çerçeve içerisine yerleştirilecek fotoğrafınızı yükleyin ve 3D canlı model üzerinde inceleyin.</p>
            </div>
        ...
        <div class="flex-1 bg-stone-100 relative overflow-hidden flex items-center justify-center cursor-grab active:cursor-grabbing" id="workspaceContainer">
            <!-- Uploaded Photo Status Badge -->
            <div id="modalPhotoUploadedBadge" class="hidden absolute top-4 left-4 z-30 bg-green-600 text-white font-extrabold text-xs px-3.5 py-2 rounded-xl shadow-lg flex items-center gap-2">
                <i class="fa-solid fa-circle-check text-sm"></i> Fotoğraf Yüklendi ve 3D Çerçeveye Uygulandı
            </div>
        </div>
        ...
        <div class="p-5 border-t border-gray-100 bg-white flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <button type="button" onclick="document.getElementById('customImageInput').click()" class="bg-[#C87A53] hover:bg-[#A65F38] text-white font-bold py-3 px-6 rounded-xl transition shadow-md flex items-center justify-center gap-2 text-sm w-full sm:w-auto">
                    <i class="fa-solid fa-camera text-base"></i> 
                    <span id="uploadBtnText">Fotoğraf Yükle / Değiştir</span>
                </button>
            </div>
            
            <div class="flex items-center gap-3 w-full sm:w-auto justify-end">
                <button type="button" onclick="closeCustomizationModal()" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 px-5 rounded-xl transition text-sm">
                    Kapat
                </button>
                <button type="button" onclick="submitCustomizedForm()" class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-xl transition shadow-md text-sm flex items-center justify-center gap-2 w-full sm:w-auto">
...
function buildModalFrame() {
    ...
    const bScale = <?php echo e($product->threeDTemplate->bump_scale ?: 0.08); ?>;
...
function handleImageUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        // Show uploaded status badge
        const badge = document.getElementById('modalPhotoUploadedBadge');
        if (badge) badge.classList.remove('hidden');

        const uploadText = document.getElementById('uploadBtnText');
        if (uploadText) uploadText.innerText = "Fotoğrafı Değiştir";
    };
    reader.readAsDataURL(file);
}
