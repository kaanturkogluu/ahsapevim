<?php $__env->startSection('title', 'Hesabım & Profilim - AhşapEvim'); ?>
    ...
    <div class="bg-gradient-to-r from-[#FAF3EE] via-[#F7F5F0] to-[#FAF3EE] border border-amber-200/80 rounded-2xl p-6 mb-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-xl md:text-2xl font-extrabold text-gray-800 flex items-center gap-2">
                    Hoş Geldiniz, <?php echo e($user->name); ?>
                ...
                <p class="text-xs text-gray-500 mt-1 flex items-center gap-3">
                    <span><i class="fa-solid fa-envelope text-amber-700/60 mr-1"></i><?php echo e($user->email); ?></span>
                    <span><i class="fa-solid fa-calendar-day text-amber-700/60 mr-1"></i>Üyelik: <?php echo e($user->created_at?->format('d.m.Y') ?? '-'); ?></span>
                </p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 w-full md:w-auto justify-around md:justify-end">
            <div class="bg-white/90 border border-amber-100 px-4 py-2.5 rounded-xl text-center shadow-2xl shadow-gray-100">
                <span class="block text-lg font-black text-[#C87A53]"><?php echo e($orders->count()); ?></span>
                <span class="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Sipariş</span>
            </div>
            <div class="bg-white/90 border border-amber-100 px-4 py-2.5 rounded-xl text-center shadow-2xl shadow-gray-100">
                <span class="block text-lg font-black text-red-500"><?php echo e($favorites->count()); ?></span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-3 bg-white rounded-2xl border border-gray-200/80 p-3 shadow-sm space-y-1">
            <button onclick="switchTab('siparisler')" id="tab-btn-siparisler" class="tab-button w-full flex items-center justify-between p-3 rounded-xl text-xs font-extrabold text-gray-600 hover:bg-amber-50 hover:text-[#C87A53] transition">
                ...
                <span class="px-2 py-0.5 bg-orange-100 text-[#C87A53] rounded-full text-[10px] font-bold"><?php echo e($orders->count()); ?></span>
            </button>
            ...
            <button onclick="switchTab('favoriler')" id="tab-btn-favoriler" class="tab-button w-full flex items-center justify-between p-3 rounded-xl text-xs font-extrabold text-gray-600 hover:bg-amber-50 hover:text-[#C87A53] transition">
                ...
                <span class="px-2 py-0.5 bg-red-100 text-red-600 rounded-full text-[10px] font-bold"><?php echo e($favorites->count()); ?></span>
            </button>
        ...
        <div class="lg:col-span-9">
            ...
            <div id="tab-content-siparisler" class="tab-content hidden space-y-4">
                <div class="bg-white p-5 rounded-2xl border border-gray-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <a href="<?php echo e(url('/urunler')); ?>" class="py-2.5 px-5 bg-[#C87A53] hover:bg-[#A65F38] text-white font-extrabold rounded-xl text-xs transition inline-flex items-center gap-2 shrink-0 shadow-sm self-start sm:self-auto">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm hover:shadow-md transition">
                        ...
                        <div class="bg-gray-50 p-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3">
                            <div class="flex items-center gap-4">
                                <div>
                                    ...
                                    <span class="text-sm font-black text-gray-800">#<?php echo e($order->id); ?></span>
                                </div>
                                ...
                                <div class="hidden sm:block">
                                    ...
                                    <span class="text-xs font-semibold text-gray-700"><?php echo e($order->created_at?->format('d.m.Y H:i') ?? '-'); ?></span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-4">
                                <div class="text-right">
                                    ...
                                    <span class="text-sm font-black text-[#C87A53]">₺<?php echo e(number_format($order->total_amount, 2, ',', '.')); ?></span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <?php elseif(in_array($order->status, ['completed', 'delivered', 'teslim_edildi', 'teslimedildi'])): ?>
                                    ...
                                    <?php else: ?>
                                        <span class="px-3 py-1 bg-red-100 text-red-700 rounded-full font-extrabold text-xs inline-flex items-center gap-1">
                                            <i class="fa-solid fa-circle-xmark text-[10px]"></i> İptal / <?php echo e($order->payment_error_reason ?: 'Yetersiz Bakiye'); ?>
                                ...
                                <div class="py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-4">
                                        <?php if($item->product && $item->product->image): ?>
                                            <img src="<?php echo e(url($item->product->image)); ?>" alt="<?php echo e($item->product->name); ?>" class="w-14 h-14 object-cover rounded-xl border border-gray-100 shrink-0">
                                        <?php else: ?>
                                        ...
                                        <div>
                                            <h4 class="text-xs font-bold text-gray-800">
                                                <?php echo e($item->product ? $item->product->name : 'Silinmiş Ürün'); ?>

                                            </h4>
                                            <div class="text-[11px] text-gray-500 mt-0.5">
                                                Adet: <span class="font-bold text-gray-700"><?php echo e($item->quantity); ?></span> × ₺<?php echo e(number_format($item->price, 2, ',', '.')); ?>
                                            ...
                                            <?php if(!empty($item->features['is_gift']) || !empty($item->features['gift_note'])): ?>
                                                <div class="mt-1 font-bold text-amber-900 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded text-[10px] inline-flex items-center gap-1">
                                                    <i class="fa-solid fa-gift text-brand"></i> Hediye Notu: <?php echo e($item->features['gift_note'] ?: 'Hediye Paketi Yapılacak'); ?>
                                            ...
                                            <?php if($fImg || $bImg): ?>
                                                <div class="mt-1.5 flex flex-wrap items-center gap-2">
                                                    <?php if($fImg): ?>
                                                        <a href="<?php echo e(str_starts_with($fImg, 'http') ? $fImg : url($fImg)); ?>" download target="_blank" class="inline-flex items-center gap-1 bg-amber-50 border border-amber-200/80 px-2 py-0.5 rounded text-[10px] text-amber-900 font-bold hover:bg-amber-100 transition">
                                                            <i class="fa-solid fa-camera text-[#C87A53]"></i> 1. Ön Yüz Fotoğrafı
                                                        </a>
                                                    <?php endif; ?>
                                                    <?php if($bImg): ?>
                                                        <a href="<?php echo e(str_starts_with($bImg, 'http') ? $bImg : url($bImg)); ?>" download target="_blank" class="inline-flex items-center gap-1 bg-emerald-50 border border-emerald-200/80 px-2 py-0.5 rounded text-[10px] text-emerald-900 font-bold hover:bg-emerald-100 transition">
                        ...
                        <div class="bg-gray-50/70 p-3 px-4 border-t border-gray-100 text-[11px] text-gray-600 flex flex-wrap items-center justify-between gap-2">
                            <div>
                                ...
                                <span class="font-bold text-gray-700">Teslimat Adresi:</span> <?php echo e(Str::limit($order->address, 60)); ?>

                            </div>
                            <div class="flex items-center gap-3">
                                <span class="text-gray-400 font-mono text-[10px]">Alıcı: <?php echo e($order->name); ?> (<?php echo e($order->phone); ?>)</span>
                                <?php if($order->status === 'pending'): ?>
                                    <form action="<?php echo e(route('orders.cancel', $order->id)); ?>" method="POST" onsubmit="return confirm('Bu siparişi iptal etmek istediğinize emin misiniz?')">
                                        <?php echo csrf_field(); ?>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="bg-white p-12 rounded-2xl border border-gray-200/80 text-center shadow-sm">
                        ...
                        <a href="<?php echo e(url('/urunler')); ?>" class="inline-flex items-center gap-2 py-2.5 px-6 bg-[#C87A53] hover:bg-[#A65F38] text-white font-extrabold text-xs rounded-xl transition shadow-sm">
            ...
            <div id="tab-content-favoriler" class="tab-content hidden space-y-4">
                <div class="bg-white p-5 rounded-2xl border border-gray-200/80 shadow-sm flex items-center justify-between">
                    ...
                    <span class="text-xs font-bold text-gray-500"><?php echo e($favorites->count()); ?> Ürün</span>
                </div>

                <?php if($favorites->count()): ?>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <?php $__currentLoopData = $favorites; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $favProduct): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="bg-white rounded-2xl border border-gray-200/80 p-3 shadow-sm hover:shadow-md transition flex flex-col justify-between">
                                <div>
                                    <div class="relative rounded-xl overflow-hidden mb-3 aspect-square bg-gray-100">
                                        <img src="<?php echo e(url($favProduct->image)); ?>" alt="<?php echo e($favProduct->name); ?>" class="w-full h-full object-cover">
                                    </div>
                                    <h4 class="text-xs font-extrabold text-gray-800 line-clamp-1 mb-1">
                                        <?php echo e($favProduct->name); ?>

                                    </h4>
                                    <div class="text-sm font-black text-[#C87A53] mb-3">
                                        ₺<?php echo e(number_format($favProduct->price, 2, ',', '.')); ?>

                                    </div>
                                </div>

                                <div class="flex items-center gap-2 pt-2 border-t border-gray-100">
                                    <a href="<?php echo e(url('/urun/' . ($favProduct->slug ?: $favProduct->id))); ?>" class="flex-1 text-center py-2 px-3 bg-[#C87A53] hover:bg-[#A65F38] text-white font-extrabold text-xs rounded-xl transition">
                                        İncele
                                    </a>
                                </div>
                            </div>
            ...
            <div id="tab-content-bilgiler" class="tab-content hidden">
                <div class="bg-white p-6 rounded-2xl border border-gray-200/80 shadow-sm">
                    ...
                    <form action="<?php echo e(route('profile.updateInfo')); ?>" method="POST" class="mt-4 space-y-4 max-w-xl">
                        <?php echo csrf_field(); ?>

                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Adınız Soyadınız *</label>
                            <input type="text" name="name" required value="<?php echo e(old('name', $user->name)); ?>" class="w-full text-xs border border-gray-300 rounded-xl p-3 focus:border-[#C87A53] focus:ring-0 outline-none">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">E-Posta Adresiniz *</label>
                            <input type="email" name="email" required value="<?php echo e(old('email', $user->email)); ?>" class="w-full text-xs border border-gray-300 rounded-xl p-3 focus:border-[#C87A53] focus:ring-0 outline-none">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Telefon Numarası</label>
                            <input type="text" name="phone" value="<?php echo e(old('phone', $user->phone)); ?>" placeholder="05XX XXX XX XX" class="w-full text-xs border border-gray-300 rounded-xl p-3 focus:border-[#C87A53] focus:ring-0 outline-none">
            ...
            <div id="tab-content-adres" class="tab-content hidden">
                <div class="bg-white p-6 rounded-2xl border border-gray-200/80 shadow-sm">
                    ...
                    <form action="<?php echo e(route('profile.updateAddress')); ?>" method="POST" class="space-y-4 max-w-xl">
                        <?php echo csrf_field(); ?>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-1">İl (Şehir) *</label>
                                <input type="text" name="city" required value="<?php echo e(old('city', $user->city)); ?>" placeholder="Örn: Manisa" class="w-full text-xs border border-gray-300 rounded-xl p-3 focus:border-[#C87A53] focus:ring-0 outline-none">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-1">İlçe</label>
                                <input type="text" name="district" value="<?php echo e(old('district', $user->district)); ?>" placeholder="Örn: Şehzadeler" class="w-full text-xs border border-gray-300 rounded-xl p-3 focus:border-[#C87A53] focus:ring-0 outline-none">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">Açık Adres (Mahalle, Cadde, Sokak, No, Daire) *</label>
                            <textarea name="address" rows="3" required placeholder="Açık teslimat adresinizi yazınız..." class="w-full text-xs border border-gray-300 rounded-xl p-3 focus:border-[#C87A53] focus:ring-0 outline-none"><?php echo e(old('address', $user->address)); ?></textarea>
                        </div>
            ...
            <div id="tab-content-sifre" class="tab-content hidden">
                <div class="bg-white p-6 rounded-2xl border border-gray-200/80 shadow-sm">
                    ...
                    <form action="<?php echo e(route('profile.updatePassword')); ?>" method="POST" class="mt-4 space-y-4 max-w-xl">
                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/u111121823/domains/ahsapevimmanisa.com/public_html/resources/views/profile/index.blade.php ENDPATH**/ ?>