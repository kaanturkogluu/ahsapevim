<?php $__env->startSection('header', 'E-Posta Gönderim Logları & Manuel Gönderim'); ?>
...
<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-200 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-4">
        <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
            <a href="<?php echo e(route('admin.mail_logs.index')); ?>" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition <?php echo e(!request('status') ? 'bg-[#29221C] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'); ?>">
                Tüm Gönderimler
            </a>
            <a href="<?php echo e(route('admin.mail_logs.index', ['status' => 'success'])); ?>" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition <?php echo e(request('status') === 'success' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'); ?>">
                <i class="fa-solid fa-check text-[10px] mr-1"></i> Başarılılar
            </a>
            <a href="<?php echo e(route('admin.mail_logs.index', ['status' => 'failed'])); ?>" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition <?php echo e(request('status') === 'failed' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-700 hover:bg-rose-100'); ?>">
                <i class="fa-solid fa-xmark text-[10px] mr-1"></i> Başarısızlar
            </a>
        </div>

        <form action="<?php echo e(route('admin.mail_logs.index')); ?>" method="GET" class="flex items-center gap-2 w-full md:w-auto">
            <?php if(request('status')): ?>
                <input type="hidden" name="status" value="<?php echo e(request('status')); ?>">
            <?php endif; ?>
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="E-posta, konu, sipariş no..." class="w-full md:w-64 text-xs border border-gray-300 rounded-xl px-3 py-2 outline-none focus:border-[#C87A53]">
                ...
                <tbody class="divide-y divide-gray-100 font-medium text-gray-700">
                    <?php $__empty_1 = true; $__currentLoopData = $logs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50/70 transition">
                            ...
                            <td class="p-4 max-w-xs truncate" title="<?php echo e($log->subject); ?>">
                            ...
                            <td class="p-4 text-right space-x-1 whitespace-nowrap">
                                <button type="button" onclick="viewMailDetail(<?php echo e(json_encode($log)); ?>)" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-bold transition">
...
<div id="sendMailModal" class="fixed inset-0 z-[99999] bg-black/70 backdrop-blur-xs hidden flex-col items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-gray-200 relative">
        ...
        <form action="<?php echo e(route('admin.mail.send_manual')); ?>" method="POST" class="space-y-4">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ahsapevim\resources\views/admin/logs/mail.blade.php ENDPATH**/ ?>